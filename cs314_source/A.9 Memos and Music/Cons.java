...
// empty buffers into a music string
    ...
    public static Cons solve(Cons e, String v) { // edit original solve
    	Object right = rhs(e);
    	Object left = lhs(e);
    	...
    	if (right.equals(v)) {
    		...
    		Cons solving = algpats;
 			while (solving != null) {
 				Cons first = (Cons) first(solving);
 				if (transform(first, e) != null) {
 					if (solve((Cons) transform(first, e), v) != null) {
 						return solve((Cons) transform(first, e), v);
 					}
 				}
 				solving = rest(solving);
 			}
 		}
 		return null;
 	}
    ...
    public static int conshash(Object e) {
    	...
    	if (consp(e)) {
    		Cons cons = (Cons) e;
    		return conshash(first(cons)) * 17 ^ conshash(rest(cons)) * 127;
    	}
    	return e.hashCode();
    }
    ...
    public static int totaltime(Cons action) {
    	...
    	if (rest == null && consp(first)) {
    		return totaltime((Cons) first);
    	}
    	if (action == null || first == null || rest == null) {
    		return 0;
    	}
    	if (first.equals("seq")) {
    		int count = 0;
    		while (rest != null) {
    			count += totaltime((Cons) first(rest));
    			rest = rest(rest);
    		} 
    		return count;
    	}
    	if (first.equals("sync")) {
    		int count = 0;
    		while (rest != null) {
    			int time = totaltime((Cons) first(rest));
    			if (time > count) count = time;
    			rest = rest(rest);
    		} 
    		return count;
    	}
    	if (first.equals("repeat")) {
    		return (int) first(rest) * totaltime(rest(rest));
    	}
    	if(first.equals("piano")) {
    ...
    // execute an event
    public static void execute(PriorityQueue pq, Cons act, int time) { // Question #4
        ...
        if (command.equals("rest")) {
        	time += (int)(Integer)second(act); // check if its correct
        	return;
        }
       ...
       if ( command.equals("kaboom")) { 
    	   int newTime = (int) second(act);
    	   execute (pq,list("seq", list("rest", newTime/4), list("boom", newTime/4), list("boom", newTime/2)), time);
    	   return;
        }
        if ( command.equals("tambourine")) {
        	int newDuration = (int)((Integer)second(act) * random.nextDouble());
        	if (newDuration >= 2) {
        		int duration = time + (int)(Integer)second(act) - newDuration;
        		addevent(pq, list("tambourine", newDuration), duration); // schedules tambourine calling random.nextDouble()
        	}
        	emit(11, time, 2);
        	
        }
        if ( command.equals("piano")) {
        	emitp((int)(Integer)second(act), (String)third(act), time, (int)fourth(act));
        }
        // sync, seq, and repeat will not emit, but will call addevent
        // to schedule their sub-events.
        if ( command.equals("seq")) { // done
        	act = rest(act);
        	while (act != null) {
        		
        		addevent(pq, (Cons)first(act), time);
        		time += totaltime((Cons)first(act));
        		act = rest(act);
        	}
        	return;
        }
        if ( command.equals("sync")) { //done
        	while (act != null) {
        		act = rest(act);
        		addevent(pq, (Cons) first(act), time);
        	}
        ...
        if ( command.equals("repeat")) { // done
        	int n = 0;
        	while (n != (int)second(act)) {
    ...
    public static Cons round( Cons melody ) { // Question #6
    	if (!consp(melody)) {
    		return null;
    	}
    	int newTime = totaltime(melody);
    	Object first = subst(0, "?i", melody);
    	Object second = subst(1, "?i", melody);
    	Object third = subst(2, "?i", melody);
    	return list("sync", first, list("seq", list("rest", newTime/4), second), list("seq", list("rest", newTime/2), third));
    }
