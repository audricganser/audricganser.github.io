    ...
    // ****** your code starts here ******
     ...
     // 2. letter count, case-insensitive
        ...
        final Mapper map2 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	value = value.toLowerCase(); // ask if we need to make them lower case
                String words = value;
                while (words.length() != 0) {
                	char letter = words.charAt(0);
                	if(Character.isLetter(letter)) {                			 
                		mr.collect_map(Character.toString(letter), list("1"));
                	}
        ...
        final Reducer red2 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	Cons list = value;
            	while (list != null) {
                    count += Integer.decode((String) first((Cons) first(list)));
                    list = rest(list);
            	}
                mr.collect_reduce(key, new Integer(count));
                } 
            };
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map3 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	StringTokenizer token = new StringTokenizer(value.toLowerCase());
        ...
        final Mapper map4 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
	            while(value.contains(mr.parameter())) {
	            	mr.collect_map(mr.parameter(), list(key));
	              	value = value.substring(value.indexOf(mr.parameter()) + 1);
	            }    
            } 
         };
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	Cons list = value;
            	while (list != null) {
            		result = append(result, list(Integer.decode((String) first((Cons)first(list)))));
            		list = rest(list);
            	}
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	String[] number = value.split(",");
       		 	mr.collect_map(number[0], list(number[2]));
            	
            } 
        };

        final Reducer red6 = new Reducer() { // fix
            public void reduce(String key, Cons value, MapReduce mr) {
            	int count = 0;
            	Double sum = 0.0;
            	Cons list = value;
            	while (list != null) {
            		count++;
            		sum += Integer.decode((String) first((Cons)first(list)));
            		list = rest(list);
            	}
                mr.collect_reduce(key, list(sum/count, count));
            } 
        };
