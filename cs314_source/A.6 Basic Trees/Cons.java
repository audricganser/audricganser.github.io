...
public static Cons readlist( Cons lst ) {
    ...
    // all functions that operate on trees should be recursive
public static Integer maxbt (Object tree) {
	if (consp(tree)) { 
		if(maxbt(first((Cons)tree)) < maxbt(rest((Cons)tree))) {
			return maxbt(rest((Cons)tree));
		}
		else {
...
public static Integer eval (Object tree, Cons bindings) {
	 if (consp(tree)) {	
		 ...
		 Object left = lhs((Cons) tree);
		 if (operator.equals("expt")) {
			 return pow(eval(left, bindings), eval(right, bindings));
		 }
		 else if (operator.equals("*")) {
...
public static Cons english (Object tree) {
	if(consp(tree)) {
		...
		Cons list;
		if (operator.equals("*")) {
			list = append(list("the", "product", "of"), english(left));
			list = append(list, list("and"));
			list = append(list, english(right));
			return list;
		}
		if (operator.equals("/")) {
			list = append(list("the", "quotient"), english(left));
			list = append(list, list("and"));
			list = append(list, english(right));
			return list;
		}
		if (operator.equals("expt")) {
			list = append(list("the","power", "of"), english(left));
			list = append(list, list("and"));
			list = append(list, english(right));
			return list;
		}
	    if (operator.equals("+")) {
			list = append(list("the", "sum", "of"), english(left));
			list = append(list, list("and"));
			list = append(list, english(right));
			return list;
		}
		if (operator.equals("-") && right == null) {
			return list = append(list("the", "negative", "of"), english(left));
		}
		if (operator.equals("-")) {
			list = append(list("difference", "of"), english(left));
			list = append(list, list("and"));
			list = append(list, english(right));
			return list;
		}
...
public static String tojava (Object tree) {
   return (tojavab(tree, 0) + ";");    // don't change
}

public static String tojavab (Object tree, int prec) { // p.158
	return null;
}
