...
public static final Heuristic halfass = new Heuristic() {
	public Integer fn (Vertex start, Vertex goal) {
		return start.distanceTo(goal) / 2;
	}
 };
...
public static final Heuristic randombs = new Heuristic() {
	public Integer fn (Vertex start, Vertex goal) {
		return (start.distanceTo(goal)/ rng.nextInt());
	}
 };
...
public int edgecost( Vertex start, Vertex goal) {
	Vertex p = vertex(goal.str());
	int total = 0;
	for (Edge e : p.edges) {
		Edge cost = new Edge(e.target, e.cost);
		if (e.target.str().equals(start.str())) {
			total = cost.cost;
		}
	}	
	return total;
...
public int totalcost( ) { // answer: 8061 returns total cost of all edges in the graph
	int cost = 0;
...
public Cons pathto(String city) {
	Vertex p = vertex(city);
	Cons cities = Cons.list();
	while (p != null) {
		cities = Cons.cons(p.str(), cities);
		p = p.parent;	
	}
	return cities;
}
